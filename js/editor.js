    ...
    if (deleteBtn) {
        deleteBtn.addEventListener('click', function(e) {
            ...
            if (confirm('Удалить этот блок?')) {
                if (typeof snapshotEditor === 'function') snapshotEditor('before-delete-block');
                block.remove();
...
function copyBlock(originalBlock) {
    if (typeof snapshotEditor === 'function') snapshotEditor('before-copy-block');
...
function setupSettingsPanel(block) {
    ...
    // Видео
    if (block.classList.contains('content-video') && typeof setupVideoBlock === 'function') {
        setupVideoBlock(block);
    }
    // Скачать файл
    if (block.classList.contains('content-download-file') && typeof setupDownloadFileBlock === 'function') {
...
function startImageResize(wrap, img, dir, e) {
    if (typeof snapshotEditor === 'function') snapshotEditor('before-image-resize');
    ...
    function onUp() {
        ...
        if (typeof snapshotEditorDebounced === 'function') snapshotEditorDebounced();
    }
